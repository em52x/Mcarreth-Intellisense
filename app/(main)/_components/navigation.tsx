...
export const Navigation = () => {
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      if (sidebarRef.current && navbarRef.current) { // Cambiar 'IF' a 'if'
        sidebarRef.current.style.width = `${newWidth}px`;
        navbarRef.current.style.setProperty("left", `${newWidth}px`); // Corregir 'setProperety' a 'setProperty'
      }
    };
    ...
    return (
        ...
        <aside
          ref={sidebarRef}
          className={cn(
            "group/sidebar h-full bg-[#F5F5F5] dark:bg-[#262626] overflow-x-auto relative flex w-60 flex-col z-[99999]",
        ...
        >
          ...
          <nav className="bg-transparent px-3 py-5 w-full">
             {isCollapsed && <MenuIcon onClick={resetWidth} role="button" className="h-6 w-6 text-muted-foreground" />}
          </nav>
